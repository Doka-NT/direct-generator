...
class AdGeneratorTest extends TestCase
{
    ...
    public function testRenderStringLongTextException()
    {
        ...
        $parser
            ->method('getTexts')
            ->willReturn([
                '[key] ver very very long text which must be longer than 75 symbols. It is need to this test'
            ]);
    ...
    /**
     * Тестирование возможности пропуска длинных текстов и заголовков
     */
    public function testGenerateExceptionSkipping()
    {
        $parser = $this->createParser();
        $keyword = 'keyword 1';
        $text = '[key] text';
        ...
        /* @var $parser Parser */
        $generator = $this->createAdGenerator($parser, ['renderException', 'flushOutput']);
        $generator->setSkipLong(true);
        $generator->expects($this->never())->method('renderException');
        $generator->expects($this->once())->method('flushOutput')->willReturnCallback('ob_flush');
